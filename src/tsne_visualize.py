...
def main():
    ...
    # 1. æºå?æ¨¡å?ä¸¦è??¥æ???    num_classes = 5
    ...
    print(f"?å??½å? {all_features.shape[0]} ç­è??ç¹å¾µï??¹å¾µç¶­åº¦: {all_features.shape[1]}")
    ...
    plt.title(f"t-SNE Visualization of Transformer Features (Robust)")
    plt.xlabel("t-SNE Dimension 1")
    plt.ylabel("t-SNE Dimension 2")
    plt.legend(title="Action Class")
    plt.grid(True, linestyle='--', alpha=0.5)
    
    output_path = "../tsne_visualization_robust.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"t-SNE è¦è¦º?å?è¡¨å·²?å??²å??? {output_path}")

    # 7. è¨ç?ä¸¦ç¹ªè£?Confusion Matrix
    print("æ­?¨ç¹ªè£½ Confusion Matrix...")
    all_preds = np.concatenate(preds_list, axis=0)
    cm = confusion_matrix(all_labels, all_preds)
    
    # è½æ??ºç¾?æ? (ä»¥å?/True Label ?ºåºæºå?æ­????
    cm_perc = cm.astype('float') / cm.sum(axis=1)[:, np.newaxis]
    
    plt.figure(figsize=(8, 6))
    sns.heatmap(cm_perc, annot=True, fmt='.2%', cmap='Blues')
    plt.title("Confusion Matrix (Robust Transformer)")
    plt.xlabel("Predicted Label")
    plt.ylabel("True Label")
    
    cm_output_path = "../confusion_matrix_robust.png"
    plt.savefig(cm_output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Confusion Matrix å·²æ??å²å­è³: {cm_output_path}")
